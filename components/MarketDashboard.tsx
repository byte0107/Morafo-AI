...
const MarketDashboard: React.FC<MarketDashboardProps> = ({ language }) => {
  ...
  const [regForm, setRegForm] = useState({ name: '', farmName: '', district: 'Maseru', village: '', nationalID: '', phone: '+266 ', productionType: 'Poultry' });
  ...
  const t = {
      ...
      scopeWarning: language === 'st' ? "Kenya feela likhoho kapa mebutla." : "ONLY list poultry, rabbits, or equipment.",
  ...
  const handleRegister = (e: React.FormEvent) => {
      e.preventDefault();
      if(regForm.name && regForm.phone.length > 8) {
      ...
      <div className="flex-1 overflow-y-auto pb-32 scrollbar-hide">
        {activeTab === 'prices' ? (
             <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                {/* min-w-0 and h-[450px] provide stable context for ResponsiveContainer */}
                <div className="lg:col-span-7 bg-white p-8 rounded-[40px] shadow-sm border border-slate-200 min-h-[450px] min-w-0">
                    <div className="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p className="text-xs font-bold text-slate-400 mt-1">Regional Averages</p>
                        </div>
                    </div>
                    {/* Stable Wrapper */}
                ...
                <div className="lg:col-span-5 space-y-4">
                    {data.map((item, idx) => (
                        <div key={idx} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-md flex items-center justify-between hover:border-green-400 transition-all">
                            <div className="flex items-center gap-4">
                                <div className={`p-4 rounded-2xl ${item.trend === 'up' ? 'bg-red-50 text-red-600' : item.trend === 'down' ? 'bg-green-50 text-green-600' : 'bg-slate-50 text-slate-600'}`}>
                                ...
                                <div>
                                    ...
                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.unit}</div>
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-2xl font-black text-slate-900">M {item.price.toFixed(2)}</div>
                                <div className={`text-[10px] font-black uppercase ${item.trend === 'up' ? 'text-red-500' : item.trend === 'down' ? 'text-green-500' : 'text-slate-400'}`}>
                                    {item.prediction}
                                </div>
                            </div>
                        </div>
                ...
                <div className="bg-blue-600 p-8 rounded-[40px] text-white shadow-2xl relative overflow-hidden flex flex-col sm:flex-row gap-8 justify-between items-center group">
                    ...
                    <div className="flex gap-6 relative z-10">
                        ...
                        <div>
                            ...
                            <p className="text-lg text-blue-100 font-medium">Safe trading for Lesotho producers.</p>
                        </div>
                    </div>
                    {!farmerProfile ? (
                        <button onClick={() => setShowRegisterModal(true)} className="bg-white text-blue-700 px-8 py-4 rounded-2xl text-base font-black shadow-2xl hover:bg-blue-50 transition-all relative z-10">{t.registerBtn}</button>
                    ) : (
                        <button onClick={() => setShowListingModal(true)} className="bg-green-500 text-white px-8 py-4 rounded-2xl text-base font-black shadow-2xl hover:bg-green-600 transition-all flex items-center gap-3 relative z-10"><Plus size={24} /> Post Ad</button>
                    )}
                </div>

                {loading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[1,2,3].map(i => <div key={i} className="bg-white h-64 rounded-[40px] border-2 border-slate-50 animate-pulse"></div>)}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {listings.map((item, idx) => (
                            <div key={idx} className={`bg-white p-8 rounded-[40px] border-2 relative shadow-sm hover:shadow-2xl transition-all group ${item.isUserListing ? 'border-blue-500 ring-8 ring-blue-50' : 'border-slate-50'}`}>
                                ...
                                <h4 className="font-black text-slate-900 text-xl mb-2 group-hover:text-blue-600 transition-colors">{item.item}</h4>
                                ...
                                <div className="flex items-center justify-between border-t border-slate-50 pt-6 mt-auto">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-600 font-black text-sm">
          ...
          <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4">
              <div className="bg-white rounded-[48px] w-full max-w-md p-10 shadow-2xl animate-in fade-in zoom-in duration-300">
                  <div className="flex justify-between items-center mb-8">
                      ...
                      <button onClick={() => setShowRegisterModal(false)} className="p-3 bg-slate-100 rounded-2xl hover:bg-slate-200 transition"><X size={24} /></button>
                  </div>
                  <form onSubmit={handleRegister} className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2 block">Name</label>
                        <input required className="w-full p-5 bg-slate-100 rounded-3xl border-2 border-slate-200 text-slate-900 font-black focus:border-green-500 focus:bg-white transition-all outline-none" placeholder="Full name..." value={regForm.name} onChange={e => setRegForm({...regForm, name: e.target.value})} />
                      </div>
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2 block">District</label>
                           <select className="w-full p-5 bg-slate-100 rounded-3xl border-2 border-slate-200 text-slate-900 font-black outline-none" value={regForm.district} onChange={e => setRegForm({...regForm, district: e.target.value})}>
                              {DISTRICTS.map(d => <option key={d} value={d}>{d}</option>)}
                           </select>
                        </div>
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2 block">Phone</label>
                           <input required className="w-full p-5 bg-slate-100 rounded-3xl border-2 border-slate-200 text-slate-900 font-black outline-none" placeholder="+266..." value={regForm.phone} onChange={e => setRegForm({...regForm, phone: e.target.value})} />
                        </div>
                      </div>
                      <button type="submit" className="w-full py-6 bg-green-600 text-white rounded-[32px] font-black shadow-2xl hover:bg-green-700 transition-all uppercase tracking-[0.2em] mt-6">{t.submitReg}</button>
                  </form>
              </div>
          </div>
      )}
          ...
          <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4">
              <div className="bg-white rounded-[48px] w-full max-w-md p-10 shadow-2xl animate-in fade-in zoom-in duration-300">
                  <div className="flex justify-between items-center mb-8">
                      ...
                      <button onClick={() => setShowListingModal(false)} className="p-3 bg-slate-100 rounded-2xl hover:bg-slate-200 transition"><X size={24} /></button>
                  </div>
                  <form onSubmit={handleAddListing} className="space-y-6">
                      <div className="flex bg-slate-100 p-2 rounded-3xl">
                          <button type="button" onClick={() => setListForm({...listForm, type: 'selling'})} className={`flex-1 py-4 text-xs font-black rounded-2xl transition-all ${listForm.type === 'selling' ? 'bg-white shadow-xl text-green-700' : 'text-slate-500'}`}>Selling</button>
                          <button type="button" onClick={() => setListForm({...listForm, type: 'buying'})} className={`flex-1 py-4 text-xs font-black rounded-2xl transition-all ${listForm.type === 'buying' ? 'bg-white shadow-xl text-orange-700' : 'text-slate-500'}`}>Buying</button>
                      </div>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2 block">Item</label>
                        <input required className="w-full p-5 bg-slate-100 rounded-3xl border-2 border-slate-200 text-slate-900 font-black outline-none" placeholder="What are you selling/buying?" value={listForm.item} onChange={e => setListForm({...listForm, item: e.target.value})} />
                      </div>
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2 block">Price (M)</label>
                          <input required type="number" className="w-full p-5 bg-slate-100 rounded-3xl border-2 border-slate-200 text-slate-900 font-black outline-none" placeholder="0.00" value={listForm.price} onChange={e => setListForm({...listForm, price: e.target.value})} />
                        </div>
                        <div className="space-y-2">
                           <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2 block">Village</label>
                           <input required className="w-full p-5 bg-slate-100 rounded-3xl border-2 border-slate-200 text-slate-900 font-black outline-none" placeholder="Location" value={listForm.location} onChange={e => setListForm({...listForm, location: e.target.value})} />
                        </div>
                      </div>
                      <button type="submit" className="w-full py-6 bg-blue-600 text-white rounded-[32px] font-black shadow-xl hover:bg-blue-700 transition-all uppercase tracking-[0.2em] mt-6">Publish Ad</button>
                  </form>
              </div>
          </div>
      )}
